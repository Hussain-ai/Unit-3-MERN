...
function App() {
  ...
  // handle the nested state
    ...
    const updatePaid = (e) => {
      // let copyState = [...state]
      // let updatedRec = {...copyState[parseInt(e.target.id)+1]}
      // updatedRec.paid = true
      
      // copyState[parseInt(e.target.id)+1] = updatedRec
      // setState({
      //   copyState
      // })
    ...
    <>
      ...
      <main>
        <div className='recipesContainer'>
          {state.map((p, idx) => {
            if(!p.paid){
              return <Receipt order={p} key={idx} func={updatePaid}/>
            }
